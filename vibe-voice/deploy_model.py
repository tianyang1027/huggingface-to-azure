...
model_id = "microsoft/VibeVoice"

# Generate endpoint and deployment names
os.environ["ENDPOINT_NAME"] = f"VibeVoice-endpoint-{str(uuid4())[:8]}"
os.environ["DEPLOYMENT_NAME"] = f"VibeVoice-deployment-{str(uuid4())[:8]}"
...
registered_model = ml_client.models.get(name="VibeVoice", version=1)
print(f"✅ Model retrieved: {registered_model.name}, version={registered_model.version}")

# Get environment
print("📥 Retrieving environment...")
env = ml_client.environments.get(name="vibe-voice-env", version=2)
